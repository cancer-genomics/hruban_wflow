...
Multivariate Cox proportional hazard analyses were generated for each molecular method and fit to overall survival adjusting for clinical subgroups. Each of the indicated subgroups that have been shown to be significant on univariate analyses have been included in the multivariate analysis.  Hazard models are shown using ARTEMIS-DELFI scores at baseline and after one cycle of treatment for patients in the PACTO study.
...
blood_draw_bl_or_p1_p <- blood_draw_p %>%
  filter(!str_detect(cgid, "89[4,5]P.*_.*")) %>% # removing duplicate samples
...
```{r fig9b, fig.width = 12, fig.height = 5, dev = c("png", "pdf")}
...
os_mf_p <- coxph(Surv(os, death) ~ age_bin + #ecog_ps_bin +
                   #weightloss5_yn_bin +
                   mgps_bin + bilirubin_bin + arm + baseline + week2,
...
```{r fig9b-pfs, fig.width = 12, fig.height = 5, dev = c("png", "pdf"), eval = FALSE}
