---
title: Heat map of clinical features and cfDNA fragmentation and genomic repeat features
---


The vertical axis is categorized by all patients with plasma samples at baseline, and at follow-up for each of the clinical RECIST 1.1 response categories and sorted by ARTEMIS-DELFI scores in descending order. All molecular features evaluated are plotted along the horizontal axis and colored by feature type. The heatmap color scale reflects the deviation of cfDNA features as compared to the mean of non-cancer individuals.
...
# Load metadata
clean_data <- read_excel(here("data/supplementary_tables.xlsx"), sheet="Table S1",range="A2:V45")
dm.meta <- read_excel(here("data/supplementary_tables.xlsx"), sheet="Table S2",range="A2:N219")

# Load DELFI features
features <- read_csv(here("data/checkpac_features.csv"))
bins5mb2 <- read_csv(here("data/checkpac_bins.csv"))

# Load ARTEMIS features
artemis <- as_tibble(read_xlsx(here("data/CP_PAC_artemis.xlsx"), sheet="CPartemis")) 
e <- read_csv(here("data/Expected.csv"))
...
```{r heatmap.body}
meta<-dm.meta %>% mutate('type'= case_when((Category == 'Baseline') ~ 'CheckPAC Baseline',
                                     (Category== 'Partial Response') ~ 'CheckPAC Endpoint Partial Response',
                                     (Category== 'Stable Disease') ~ 'CheckPAC Endpoint Stable Disease',
                                     (Category=='Progressive Disease') ~'CheckPAC Endpoint Progressive Disease'))
meta<-meta %>% select(id, type)

bins5mb_checkpac<-artemis
...
dna_features <- features %>% select(41:79, 553:2362)
...
features_annot[, c('...1')] <- NULL
labs_feat<-unique(features_annot$Feature.type)
```
...
```{r fig5, fig.width = 20, fig.height = 8, dev = c("png", "pdf")}
...
col_fun_column = list(`Feature.type` = structure(c('turquoise4','darkorange2', 'mediumpurple3',"darkmagenta","blueviolet","mediumorchid3","maroon3"), names = c('Chromosomal Change', 'Fragmentation', 'Repeats - LINE',"Repeats - TE","Repeats - Satellite","Repeats - LTR","Epigenetic")))
